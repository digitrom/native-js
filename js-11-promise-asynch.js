...
fetch('https://google.com/?query=js')
    ...
    .then(() => { // передаем в колбэк undefined, поэтому задерки практически нету
        fetch('https://yahoo.com/?query=js')
    })
    .then(() => {
        console.log('Response from yahoo')
...
// если в response бдует error, то мы попадаем в catch:
//'Response from google'//
...
//all - дожидается всех промисов переданных в массив зависимости, и после делаем манипуляции с пришедшими данными
...
//case2
//если хоть в 1 из промисов будет ошибка, то промис зареджектится:
...
//race - обработаем тот промис, который 1-ым отработает - т.е.  promise3
